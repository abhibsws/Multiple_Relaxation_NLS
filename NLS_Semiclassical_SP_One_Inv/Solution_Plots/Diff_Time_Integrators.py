    ...
    while t < T and not np.isclose(t, T):
        ...
        if mult_fac >= 0.9:
            ...
            if Err >= 1e-12:
                ...
                dts = np.append(dts,dt) # variable time steps
                unew = unew + np.dot(gamma,inc); t+=(1+sum(gamma))*dt; # relaxation solution
