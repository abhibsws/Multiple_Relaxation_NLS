...
def Compute_Sol_ImEx_Relaxation(f_stiff,f_non_stiff,IC,ep,case,xL,xR,N,t0,T,Mthdname,rkim,rkex,b,bhat,c,inv,eta1,eta2,rgam,dt,em_or,tol): 
    ...
    while t < T and not np.isclose(t, T):
        ...
        print('mult_fac=%1.4f \n'%mult_fac)
        print('Proposed dt by step size control = %1.4f \n'%dt_new)
        if mult_fac >= 0.9:
            if inv == 1:
            ...
            if Err >= 1e-12:
                ...
                if inv == 1:
                    print("Relaxation %s: At step no = %d (time = %1.5f), ier = %d and γ1 = %1.6f"%(Mthdname,steps,tt[-1],ier,sum(gamma)))
                elif inv == 2:
                    print("Relaxation %s: At step no = %d (time = %1.5f), ier = %d and γ1+γ2  = %1.6f"%(Mthdname,steps,tt[-1],ier,sum(gamma)))
        else: 
    ...
    return dts, tt, uu, G, no_ier_one, no_ier_five, no_ier_else
...
def Op_Split_Exact_Solve(nonlin_f,sol_nl_part,sol_l_part,IC,ep,case,xL,xR,N,t0,T,Mthdname,a,b,dt):
    ...
    while t < T and not np.isclose(t, T):
        ...
        tt = np.append(tt, t); uu = np.append(uu, np.reshape(unew.copy(), (1,len(unew))), axis=0)  
        steps += 1
        print("Method = %s: Step number = %d (time = %1.2f)"%(Mthdname,steps,tt[-1]))
    return tt, uu
