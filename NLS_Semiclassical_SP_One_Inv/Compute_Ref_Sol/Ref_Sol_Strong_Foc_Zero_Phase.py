...
# Different cases
case = 'strong_cubic_foc_zero_phase'; case_title = 'Strong Cubic Foc Zero Phase';
xL = -8; xR =8; L = xR-xL; N_ref = L*4096; dt_ref = 1e-4; t0 = 0; 
Eps = np.array([0.2,0.2/2,0.2/4]); Ts = np.array([0.8,1.2])
# Operator splitting methods
OS_method_names = ['Op_Sp1','Op_Sp2','Op_Sp4']; OS_Stage = [1,2,5]; OS_Order = [1,2,4]; OS_Sch_No = [0,1,2];



St_Defoc = np.empty((len(Eps), len(Ts), 2*N_ref)); OS_idx = 2;
...
df.to_csv(filepath)         
np.save("./%s_RefSol_N_%d_dt_%1.e.npy"%(case,N_ref,dt_ref), St_Defoc)
